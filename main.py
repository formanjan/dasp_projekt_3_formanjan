"""
main.py: Datový analytik s Pythonem - Projekt 3
...
### ÚPRAVA PO 1. ODEVZDÁNÍ
def vycisti_text(text):
    """
    Odstraní z textu nedělitelnou mezeru a běžné mezery.
    """
    return text.replace('\xa0', '').replace(' ', '')


def ziskej_statistiky(soup):
    """
    Získá z HTML tabulky základní statistiky o voličích a hlasech v obci.
    Vrací trojici hodnot: počet registrovaných voličů, vydaných obálek a platných hlasů.
    Pokud chybí očekávané buňky, vrací None.
    """
    stats_table = soup.find_all('table')[0]  # Najde se tabulka se statistikami
    stats_cells = stats_table.find_all('td')  # Najdou se buňky tabulky
    if len(stats_cells) < 8:
        return None  # Zkontroluje se dostatečný počet údajů
    registered = vycisti_text(stats_cells[3].text)  # Počet registrovaných voličů
    envelopes = vycisti_text(stats_cells[4].text)  # Počet vydaných obálek
    valid = vycisti_text(stats_cells[7].text)  # Počet platných hlasů
    return registered, envelopes, valid


def ziskej_hlasy_stran(soup):
    """
    Získá počet hlasů pro každou stranu z HTML tabulek.
    Vrací slovník s názvy stran jako klíče a počty hlasů jako hodnoty.
    """
    ...
    for table in vote_tables: # Projde se každá tabulka
        ...
        for row in rows: # Projde se každý řádek
            ...
            if len(cells) >= 2: # Zkontroluje se minimální počet buněk
                ...
                vote_count = vycisti_text(cells[2].text).replace('-', '0')  # Získá počet hlasů a nahradí pomlčky nulou (problém s testovacími samples viz Jeseník)
...
def ziskej_vysledky_pro_obec(code, name, link, sorted_party_names):
    ...
    statistiky = ziskej_statistiky(soup)  # Získání základních údajů
    if not statistiky:
        log(f"⚠️ U obce {name} ({code}) chybí statistiky, přeskočeno.")
        return None, sorted_party_names
    registered, envelopes, valid = statistiky

    parties = ziskej_hlasy_stran(soup)  # Získání počtů hlasů pro strany
...
def zpracuj_obce(obec_links, sorted_party_names):
    """
    Zpracuje všechny obce – získá výsledky, sleduje chyby.
    Vrací:
        - seznam výsledků pro všechny obce,
        - aktualizovaný seznam názvů stran,
        - počet obcí, které se nepodařilo zpracovat.
    """
    ...
    return vysledky, sorted_party_names, chyby
...
def main():
    ...
    vysledky, sorted_party_names, chyby = zpracuj_obce(obec_links, sorted_party_names)
